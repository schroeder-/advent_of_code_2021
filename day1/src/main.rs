...
fn calc_increased_measure(data: &[i32]) -> u32{
    data.iter().skip(1).fold((0, data[0]), |(cnt, prev), item| {
        if *item > prev{
            (cnt + 1, *item)
        } else{
            (cnt, *item)
...
fn load_file<P>(filename: P) -> io::Result<Vec<i32>> where P: AsRef<Path>{
    let file = File::open(filename)?;
    Ok(io::BufReader::new(file).lines().map(|l| l.unwrap().parse::<i32>().unwrap()).collect())
}
